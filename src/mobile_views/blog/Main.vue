<template>
  <div :style="'background: #f8f8f8;min-height: '+windowSize.height+'px'">
    <van-nav-bar right-text="分享"
                 style="position:fixed;top:0;z-index: 9999; box-shadow: 0px -3px 10px #888888;width: 100%;" title="博客列表"
    ...
    <router-link v-for="(item,index) in blogs" :key="'p'+index" :to="`/mobile/user/blog/details/${item.id}`">
      ...
      <van-panel :desc="'更新时间 '+item.updateTime"
                 :title="item.title"
                 style="
                  background: #fff;
                  width: 90%;
                  ...
                  border-bottom: none;">
        <div style="padding: 7px 15px 7px 15px;color: #303133;font-size: 0.9rem; color: #888;">
          {{ $util.cutStr(item.description, 50) }}
        </div>
      </van-panel>
    </router-link>
